...
const layoutProvider = new LayoutProvider(() => 'page', (_type, dim) => {
    dim.width = constants.screenWidth;
    dim.height = constants.screenHeight;
});
const dataProviderMaker = (items) => new DataProvider((item1, item2) => item1.value !== item2.value || item1.label !== item2.label).cloneWithRows(items);
const HorizontalList = (props, ref) => {
    const { renderItem, data, pageWidth = constants.screenWidth, onPageChange, initialPageIndex = 0, extendedState, scrollViewProps } = props;
    ...
    return (<RecyclerListView ref={ref} isHorizontal rowRenderer={renderItem} dataProvider={dataProvider} layoutProvider={layoutProvider} extendedState={extendedState} initialRenderIndex={initialPageIndex} renderAheadOffset={5 * pageWidth} onScroll={onScroll} scrollViewProps={{
            pagingEnabled: true,
            bounces: false,
            ...scrollViewProps
        }}/>);
