...
const TimelineList = (props) => {
    ...
    const { pages, pagesRef, resetPages, resetPagesDebounce, scrollToPageDebounce, shouldResetPages, isOutOfRange, isNearEdges } = useTimelinePages({ date, listRef });
    useEffect(() => {
    ...
    const renderPage = useCallback((_type, item) => {
        ...
        return (<>
          <Timeline {...timelineProps} key={item} date={item} scrollToFirst={false} events={timelineEvent} scrollOffset={isCurrent ? undefined : timelineOffset} onChangeOffset={onTimelineOffsetChange}/>
          {/* NOTE: Keeping this for easy debugging */}
          {/* <Text style={{position: 'absolute'}}>{item}</Text> */}
        </>);
    }, [events, timelineOffset]);
    return (<HorizontalList ref={listRef} data={pages} renderItem={renderPage} onPageChange={onPageChange} onScroll={onScroll} extendedState={{ todayEvents: events[date], pages }} initialPageIndex={INITIAL_PAGE} scrollViewProps={{
